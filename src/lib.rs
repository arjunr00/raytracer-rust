...
pub fn write_pixel(pixel: &ColorRGB, samples: u32) -> String {
    ...
    let r = pixel[vec::Color::R] * scale;
    let g = pixel[vec::Color::G] * scale;
    let b = pixel[vec::Color::B] * scale;
...
pub fn create_ppm(world: &HittableGroup, width: u32, height: u32, samples: u32) -> String {
    ...
    for i in 0..height {
        for j in 0..width {
            ...
            for _ in 0..samples {
                let u = ((j as f64) + rand_f64(&zero_to_one, &mut rng)) / f64::from(width - 1);
                let v = ((i as f64) + rand_f64(&zero_to_one, &mut rng)) / f64::from(height - 1);

                let r = camera.ray(u, v);

                color += r.get_color(world);
            }
